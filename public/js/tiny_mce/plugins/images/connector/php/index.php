...
class TinyImageManager {
	...
	function TinyImageManager() {
		...
		switch ($_POST['action']) {
			...
			case 'showtree':
				...
				else 						echo $this->DirStructure('images', 'first', $this->AccessDir($_POST['path'], 'images'));
				if($_POST['type']=='files') $this->firstAct = false;
				if($_POST['type']=='images') echo $this->DirStructure('files', 'first');
				else 						 echo $this->DirStructure('files', 'first', $this->AccessDir($_POST['path'], 'files'));
				exit();
			break;
			
			//Показать путь (хлебные крошки вверху)
			case 'showpath':
				echo $this->DirPath($_POST['type'], $this->AccessDir($_POST['path'], $_POST['type']));
				exit();
			break;
	...
	function Tree($beginFolder) {
		...
		if ($handle) {
			...
			$struct[$beginFolder]['name'] = current($tmp);//$tmp[sizeof($tmp)-1];
	...
	function DirStructure($type, $innerDirs='first', $currentDir='', $level=0) {
		...
		if(!empty($currentDir)) {
			...
			//print_r($currentDirArr);
		}
		
		if($innerDirs == 'first') {
			$innerDirs = $this->Tree($this->dir[$type]);
			//if(empty($currentDir) && !$this->firstAct) {
		...
		foreach ($innerDirs as $v) {
			...
			if(isset($v['childs'])) {
				...
				$folderAct = '';
				$folderClass = 'folderS';
				//echo $currentDirArr[$level-2].' == '.$v['name'].'+';
				...
				if($currentDirArr[sizeof($currentDirArr)]==$v['name']) {
					$folderAct = 'folderAct';
				} else {
					$folderAct = '';
				}
...
*/
				...
				if ($file != "." && $file != "..") {
					...
					if(is_file($dir.'/'.$file) ) {
						...
						if($file_info['extension']=='jpg' || $file_info['extension']=='jpeg') {
							...
							//$ret .= '<pre>'.print_r($er->getImageInfo(),1).'</pre>';
	...
	function UploadFile($dir, $type) {
		//file_put_contents($_SERVER['DOCUMENT_ROOT'].'/r_'.rand(1,1000).'.txt', print_r(array($_FILES, $_POST), 1));
		$dir = $this->AccessDir($dir, $type);
		if(!$dir) return false;
		...
		//Файлы из обычной загрузки 
			...
			sort($_FILES);
			$ufiles = $_FILES[0];
			//file_put_contents($_SERVER['DOCUMENT_ROOT'].'/outp.txt', print_r($ufiles, 1));
			//exit();
...
//echo '<pre>'.print_r($letsGo->CallDir('', 'images'), 1).'</pre>';
